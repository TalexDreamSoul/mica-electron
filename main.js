...
/**
 * Use mica-electron with custom path
 * @param {string} _path The path of mica-electron.node
 */
function useMicaElectron(_path = "src/micaElectron_" + process.arch + ".node") {
  if (initialized) {
    console.log("Mica-Electron: Already initialized");
    return;
  }
  initialized = true;
...
function isWin11() {
  ...
  if (version.length == 3) return version[2] >= "22000";
  return false;
}
...
class BrowserWindow extends electron.BrowserWindow {
  ...
  constructor(...args) {
    if (!initialized) {
      console.error("Mica-Electron: Not initialized");
      throw new Error("Mica-Electron: Not initialized");
    }
    if (args.length > 0) {
    ...
    this.hasFrameless =
      args[0].frame === false || args[0].titleBarStyle == "hidden";
    ...
    this.on("resize", () => {
      if (this.hasFrameless) setTimeout(applyEffect, 60); // refresh effect
    });
  }
  ...
  disableMargin() {
    ...
    if (this.useDWM) this.executeDwm(PARAMS.MARGIN, 1);
  }
  ...
  enableMargin() {
    ...
    if (!this.hasFrameless) this.executeDwm(PARAMS.MARGIN, 0);
    else
   ...
   */
  disableUser32() {
    if (!this.useDWM) this.executeUser32(0, 0xffffffff);
    this.useDWM = true;
  }
   ...
   */
  disableDWM() {
    if (this.useDWM) this.executeDwm(PARAMS.BACKGROUND.NONE, this.theme);
    this.disableMargin();
    this.useDWM = false;
  }
...
module.exports = {
  useMicaElectron: useMicaElectron,
  PARAMS: PARAMS,
  VALUE: VALUE,
